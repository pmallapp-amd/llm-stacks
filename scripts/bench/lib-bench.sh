...
# KV_TRANSPORT, KV_MAX_VALUE_SIZE, LMCACHE_CHUNK_SIZE, TP_SIZE, or even
# which git commit of THIS repo's proxy/plugin code was running at the
# time. Every field below is chosen because changing it plausibly changes
# the number: transport flips the physical wire, chunk/value-size changes
# on-wire object geometry (see config/cluster.env's KV_MAX_VALUE_SIZE
# comment), TP_SIZE changes how much HBM is left for local KV, and the git
# commit is the only thing that pins "which version of the proxy's
# sequential-priming behavior" (scripts/proxy/disagg_proxy.py) was in
# effect.
# ─────────────────────────────────────────────────────────────────────────────
