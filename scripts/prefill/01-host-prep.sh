...
# HTTP port itself (PREFILL_PORT), the NIXL side-channel port (the
...
# ─────────────────────────────────────────────────────────────────────────────
step "Firewall (vLLM ${PREFILL_PORT}, NIXL side channel ${NIXL_SIDE_CHANNEL_PORT_PREFILL}, LMCache MP ${LMCACHE_MP_PORT})"
declare -A _fw_ports=(
    ["${PREFILL_PORT}"]="vLLM HTTP (prefill)"
    ["${NIXL_SIDE_CHANNEL_PORT_PREFILL}"]="NIXL side channel <- ${DECODE_NAME}"
...
if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -q "^Status: active"; then
    ...
    info "neither ufw nor firewalld is active — no firewall changes made" \
         ...
         " ${PREFILL_PORT}/${NIXL_SIDE_CHANNEL_PORT_PREFILL}/${LMCACHE_MP_PORT}" \
