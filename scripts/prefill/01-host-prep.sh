...
# operator can pin PREFILL_UCX_NET_DEVICES to the CORRECT physical port on
# THIS host — F3: the device index is not guaranteed to line up the same
# way on SMC1 as it does on SMC2 (e.g. ionic_2 can be a different physical
...
if [ -d /sys/class/infiniband ]; then
    ...
    warn "F3 (verified 2026-09-11): cross-check the pci= addresses above" \
         ...
         " PREFILL_UCX_NET_DEVICES='${PREFILL_UCX_NET_DEVICES}' in" \
         " config/cluster.env — the same ionic_N NAME can be a different" \
         " physical port on SMC2, so a value copied verbatim from decode's" \
         " report is not safe to assume correct here."
else
...
if [ "${KV_TRANSPORT}" = "rdma" ]; then
    require_rdma_access "${PREFILL_UCX_NET_DEVICES}"
else
    shopt -s nullglob
...
# ─────────────────────────────────────────────────────────────────────────────
# Firewall — open the NIXL side-channel port to the peer (decode) node. Same
# pattern as scripts/target/01-host-prep.sh's NVMe-oF port opening: act only
# if a host firewall is actually enforcing anything, and say so plainly if
# neither ufw nor firewalld is active rather than guessing at iptables
# rules. Without this, the side-channel handshake (F2) can fail exactly the
# way an unreachable loopback default does — silently, until the peer
# actually tries to connect.
# ─────────────────────────────────────────────────────────────────────────────
step "Firewall (${NIXL_SIDE_CHANNEL_PORT_PREFILL}/tcp, NIXL side channel <- ${DECODE_NAME})"
if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -q "^Status: active"; then
    ufw allow "${NIXL_SIDE_CHANNEL_PORT_PREFILL}/tcp" comment "NIXL side channel (kvstack, prefill)"
    ok "ufw: opened ${NIXL_SIDE_CHANNEL_PORT_PREFILL}/tcp"
elif command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active --quiet firewalld 2>/dev/null; then
    firewall-cmd --permanent --add-port="${NIXL_SIDE_CHANNEL_PORT_PREFILL}/tcp"
    firewall-cmd --reload
    ok "firewalld: opened ${NIXL_SIDE_CHANNEL_PORT_PREFILL}/tcp"
else
    info "neither ufw nor firewalld is active — no firewall changes made" \
         " (if some other mechanism blocks" \
         " ${NIXL_SIDE_CHANNEL_PORT_PREFILL}/tcp, e.g. raw iptables/nftables" \
