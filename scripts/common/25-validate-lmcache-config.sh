...
#        25-validate-lmcache-config.sh --l2-adapter-json <JSON>
#
# The second form validates the MP daemon's --l2-adapter spec (see
# scripts/common/start-lmcache-daemon.sh) instead of the in-process YAML —
# a different config surface entirely (see gen-lmcache-config.sh's header
# for why these two do not overlap). It parses the JSON with the SAME
# installed lmcache.v1.distributed.l2_adapters.config classes the daemon
# itself uses (get_l2_adapter_config_class()/<ConfigClass>.from_dict()), so
# a typo'd key or an unregistered adapter "type" fails here instead of
# 30 seconds into a daemon start. start-lmcache-daemon.sh calls this
# automatically before launching; run standalone any time to re-check a
# spec after an LMCache upgrade.
#
