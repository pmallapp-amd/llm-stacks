                ...
                async with session.post(
                    ...
                    if 200 <= response.status < 300 or 400 <= response.status < 500:
                        if response.status >= 400:
                            # Debug instrumentation (2026-08-18): this branch forwards
                            # the upstream body verbatim inside a 200-status
                            # StreamingResponse, so a 4xx from vLLM is otherwise
                            # invisible in this proxy's own access log.
                            body_preview = await response.read()
                            logger.error(
    ...
    async def create_completion(self, raw_request: Request):
        try:
            ...
            try:
                ...
                raise http_exc
            media_type = "text/event-stream" if request.get("stream") else "application/json"
