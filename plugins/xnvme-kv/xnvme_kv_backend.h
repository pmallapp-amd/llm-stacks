...
// cache miss rather than a failure. SCT is Generic (0) and SC is 0x87.
//
// MEASURED, not read off a spec sheet — a KV Exist against a key known to be
// absent on the Austin RAM-backed target through the kernel nvme-of initiator
// returned sct=0 sc=135 (0x87), and the same key after a store returned
// sct=0 sc=0. Getting this constant wrong is not a cosmetic bug: treat 0x87 as
...
public:
    ...
    //
    // WHY THIS IS NOT A CONSTANT. This plugin shipped with "/dev/ng0n1" as its
    // advertised default, which was correct on the single MI210 host it was
    // written for. On the Austin cluster /dev/ng0n1 is a real local Micron 7450
    // WITH DATA ON IT, and on one node the root filesystem lives on a sibling
    // namespace of the same drive. A hardcoded default there aims the KV plugin
    // at production storage. The index is not stable either: the KV controller
    // takes whatever number is free, which already differs between two
    // otherwise-identical hosts and moves across reboots.
    //
    // The test is exact rather than a guess: A KV NAMESPACE HAS NO BLOCK
    // DEVICE. The kernel refuses to build one for a non-NVM command set
    // ("block device for nsid N not supported (csi 1)") and creates only the
    // generic char node, whereas every ordinary NVM namespace has both. So
    // /dev/ngXnY with no matching /dev/nvmeXnY is a namespace the kernel could
    // not interpret — which on these hosts is the KV target and never a data
    // disk. Verified both ways on the Austin prefill node 2026-09-10.
    //
    // Deliberately conservative: if more than one candidate exists it returns
    // the lowest and logs all of them, because picking arbitrarily between two
    // unknown devices is exactly the class of mistake this is here to prevent.
    static std::string discover_kv_device();
    ...
    //
    // WHY THIS HAS TO EXIST. Without it NIXL's base class answers
    // NIXL_ERR_NOT_SUPPORTED, LMCache's NixlDynamicStorageBackend logs
    // "NIXL batched query failed: NIXL_ERR_NOT_SUPPORTED" and reports EVERY
    // lookup as a miss. This backend then stores correctly and can never read
    // anything back: writes land and are unreachable. Measured on the Austin
    // cluster 2026-09-10 — stores ran at 1.7 GB/s while every request logged
    // "LMCache hit tokens: 0, need to load: 0". It is also precisely what makes
    // P/D disaggregation impossible, because a receiver's entire job is to
    // discover keys IT NEVER WROTE, and discovery was the one missing verb.
    //
    // resp[i] is engaged (has_value()) when the key exists and std::nullopt
    // when the device reports KV_KEY_DOES_NOT_EXIST. A per-descriptor transport
    // failure is ALSO reported as nullopt: a false miss costs a recompute,
    // whereas a false hit would hand LMCache a KV block that was never written.
    // Same asymmetry the SPDK_NVMe_KV backend documents.
...
private:
    ...
    // ---- queryMem's device handle ------------------------------------------
    //
    // A SEPARATE xnvme_dev, opened on the same URI, used only by queryMem().
    //
    // It does not share a QueueWorker's handle for two reasons. The header's
    // central invariant is that an xnvme_queue is touched by exactly one
    // reactor thread; queryMem() is called on LMCache's lookup thread, which is
    // none of them. And unlike the transfer path, a lookup is issued
    // SYNCHRONOUSLY here — xnvme_cmd_ctx_from_dev() + xnvme_kvs_exist() goes
    // through the kernel's passthrough ioctl rather than the io_uring_cmd
    // submission queue, so it needs no queue at all and cannot contend with one.
    //
    // Verified empirically on the Austin cluster 2026-09-10 before this was
    // written: sync STORE/EXIST/DELETE over the kernel nvme-of initiator all
    // complete correctly on this device, and an absent key returns
    // sct=0 sc=0x87 (see XNVME_KV_SC_KEY_DOES_NOT_EXIST).
    //
    // Why sync rather than mirroring the async transfer path: a lookup batch is
    // small and metadata-only, the cost is one ioctl round trip per descriptor
    // (~150 us against a multi-second retrieve), and a blocking call cannot
    // leave a lookup pending forever the way an unreaped async completion can.
    // If a future lookup batch is ever large enough for this to matter, the
    // upgrade is a private xnvme_queue owned by this path, not sharing a
    // worker's.
    struct xnvme_dev *query_dev_ = nullptr;
    // Serialises queryMem() calls on query_dev_. libxnvme's sync path is not
    // documented as reentrant on one device handle, and a lookup is cheap
    // enough that serialising costs nothing worth measuring.
    mutable std::mutex query_mtx_;
