...
#include <algorithm>    // discover_kv_device()'s deterministic pick
#include <cerrno>
#include <chrono>
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <dirent.h>     // discover_kv_device() scans /dev
#include <sys/stat.h>
#include <sys/types.h>
#include <unistd.h>
...
// ---- KV device discovery ---------------------------------------------------
//
// See the declaration for why a hardcoded /dev/ng0n1 is actively dangerous.

std::string nixlXnvmeKvEngine::discover_kv_device() {
    DIR *dir = opendir("/dev");
    if (!dir) return {};

    std::vector<std::pair<unsigned, unsigned>> candidates;   // (ctrl, nsid)
    struct dirent *ent;
    while ((ent = readdir(dir)) != nullptr) {
        unsigned c = 0, n = 0;
        // Match exactly ng<ctrl>n<nsid>; the trailing %c guards against
        // suffixed names (partitions and the like) sneaking in.
        char tail = '\0';
        if (std::sscanf(ent->d_name, "ng%un%u%c", &c, &n, &tail) != 2) continue;

        // The discriminator: an ordinary NVM namespace also has a block device.
        char blk[64];
        std::snprintf(blk, sizeof(blk), "/dev/nvme%un%u", c, n);
        struct stat st;
        if (stat(blk, &st) == 0) continue;      // has a block device -> not KV

        candidates.emplace_back(c, n);
    }
    closedir(dir);
    ...
    if (candidates.size() > 1) {
        fprintf(stderr, "[XNVME_KV] WARNING: %zu block-device-less namespaces found:",
                candidates.size());
        for (const auto &c : candidates)
            fprintf(stderr, " /dev/ng%un%u", c.first, c.second);
        fprintf(stderr, " — using the lowest. Set NIXL_XNVME_DEV explicitly if"
                        " that is the wrong one.\n");
    }

    char picked[64];
    std::snprintf(picked, sizeof(picked), "/dev/ng%un%u",
...
nixlXnvmeKvEngine::nixlXnvmeKvEngine(const nixlBackendInitParams *init_params)
    ...
    // autodiscovery. There is deliberately no hardcoded fallback path.
    {
        ...
        if (dev_env && dev_env[0] != '\0') {
            ...
            dev_uri_ = discover_kv_device();
            if (dev_uri_.empty()) {
                fprintf(stderr, "[XNVME_KV] no device: NIXL_XNVME_DEV unset, no dev_uri "
                        "param, and no KV namespace found. A KV namespace appears as "
                        "/dev/ngXnY with NO matching /dev/nvmeXnY — if you have none, the "
                        "kernel nvme driver does not own a KV controller on this host "
                        "(check `nvme list-subsys`, and that it is not bound to vfio-pci).\n");
                initErr = true;
                return;
            }
            fprintf(stderr, "[XNVME_KV] device autodiscovered: %s "
                    "(no block device -> non-NVM command set)\n", dev_uri_.c_str());
        }
    }
...
nixl_status_t nixlXnvmeKvEngine::start_workers() {
    ...
    //
    // Non-fatal on failure: queryMem() degrades to reporting NIXL_ERR_BACKEND,
    // which LMCache already treats as "assume nothing is cached". That is the
    // pre-2026-09-10 behaviour — worse, but not broken — so a device that
    // refuses a second open should not take the whole transfer path down with it.
    {
        struct xnvme_opts qopts = xnvme_opts_default();
        qopts.async = "io_uring_cmd";
        if (nsid_ != 0) qopts.nsid = nsid_;
        query_dev_ = xnvme_dev_open(dev_uri_.c_str(), &qopts);
        if (!query_dev_) {
            XNVME_PLUGIN_ERR << "query handle xnvme_dev_open(" << dev_uri_
                             << ") failed: " << strerror(errno)
                             << " — lookups will report NOT AVAILABLE and every"
                                " read will miss";
        }
    }

    fprintf(stderr, "[XNVME_KV] %zu queue(s) ready on %s (nsid=%u), lookup=%s\n",
            num_queues_, dev_uri_.c_str(), nsid_,
            query_dev_ ? "KV Exist" : "UNAVAILABLE");
    return NIXL_SUCCESS;
}

// ---- queryMem (NVMe KV Exist) ----------------------------------------------
//
// See the declaration for why this method has to exist at all.
//
// THREADING. Called on the caller's thread (LMCache's lookup path), never on a
// reactor thread. It therefore does NOT touch any QueueWorker's queue — an
// xnvme_queue is single-threaded by construction. It issues a synchronous KV
// Exist per descriptor on a private device handle instead, serialised by
// query_mtx_.
nixl_status_t nixlXnvmeKvEngine::queryMem(const nixl_reg_dlist_t         &descs,
                                          std::vector<nixl_query_resp_t> &resp) const {
    const int n = descs.descCount();
    // Default every slot to "absent". Every failure path below therefore
    ...
    if (!query_dev_) return NIXL_ERR_BACKEND;

    std::lock_guard<std::mutex> lk(query_mtx_);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (rc == 0 && !xnvme_cmd_ctx_cpl_status(&ctx)) {
            resp[static_cast<size_t>(i)] = nixl_b_params_t{};   // present
        } else if (ctx.cpl.status.sct == 0 &&
                   ctx.cpl.status.sc  == XNVME_KV_SC_KEY_DOES_NOT_EXIST) {
            // Ordinary cache miss. Stays nullopt, and is NOT a failure — see
            // XNVME_KV_SC_KEY_DOES_NOT_EXIST for why this branch is measured
            // rather than assumed.
        } else {
            // Transport/device error. Still reported as a miss (nullopt), but
            // remembered so the caller can tell "the device says no" from "the
            // device did not answer".
            any_failed = true;
            if (xnvme_kv_debug()) {
                char keyhex[XNVME_KV_KEY_MAX_LEN * 2 + 1];
                xnvme_kv_key_hex(key, key_len, keyhex);
                fprintf(stderr,
                        "[XNVME_KV] op=exist key=%s rc=%d sct=%u sc=%u "
                        "t=complete (reported as MISS)\n",
                        keyhex, rc, (unsigned)ctx.cpl.status.sct,
                        (unsigned)ctx.cpl.status.sc);
                fflush(stderr);
            }
        }
    }
    ...
    if (xnvme_kv_debug()) {
        size_t hits = 0;
        for (const auto &r : resp) if (r.has_value()) ++hits;
        fprintf(stderr, "[XNVME_KV] queryMem n=%d hits=%zu type=%d\n",
                n, hits, static_cast<int>(descs.getType()));
        fflush(stderr);
    }
...
void nixlXnvmeKvEngine::stop_workers() {
    ...
    // queryMem()'s private handle. Closed AFTER the reactor threads are joined
    // and under the same lock queryMem() takes, so a lookup in flight on
    // another thread cannot be left holding a closed device.
    {
        std::lock_guard<std::mutex> lk(query_mtx_);
        if (query_dev_) {
            xnvme_dev_close(query_dev_);
            query_dev_ = nullptr;
        }
    }
}
