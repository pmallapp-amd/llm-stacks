...
#
# Copied verbatim (no logic changes) from the vLLM v0.26.0 image's bundled
# example, /app/vllm/examples/disaggregated/lmcache/disagg_prefill_lmcache_v1/
# disagg_proxy_server.py, pulled from the live rocm-aic:latest build
# 2026-08-21. Already supports --prefiller-host/--prefiller-port/
# --decoder-host/--decoder-port as separate hosts — no cross-host changes
# needed. Use with deploy-pd-spdk.sh's two roles:
#
#   python3 disagg_proxy_server.py --host 0.0.0.0 --port 9000 \
#       --prefiller-host <<SETUP4_PREFILL_NODE>-ip> --prefiller-port <PORT from PD_ROLE=producer> \
#       --decoder-host   <<SETUP4_DECODE_NODE>-ip> --decoder-port   <PORT from PD_ROLE=receiver>
#
# Routes each /v1/completions or /v1/chat/completions request to the prefill
# instance first (max_tokens forced to 1 — prefill only, response discarded),
# then streams the real response from the decode instance. Requires an
# OPENAI_API_KEY env var to be set (sent as a Bearer token to both upstreams;
# set to any non-empty string if the upstreams don't check it).
